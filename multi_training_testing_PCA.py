...
def train_and_evaluate_all_models(datasets, chemical_representation, input):
    ...
    results_rmse = {}
    results_mae = {}
    components_per_dataset = {}  # Dictionary to store the number of components per dataset

    for dataset_name in datasets:
        model_results_r2 = []
        model_results_rmse = []
        model_results_mae = []
        model_training_times = []  # To store the time taken for training each model
        ...
        components_per_dataset[dataset_name] = n_components  # Save the number of components per dataset
        ...
        for model_name, model in models.items():
            try:
                ...
                model_training_times.append({'Model': model_name, 'Training Time (s)': training_time})
        ...
        # Add the model training times to the components_per_dataset dictionary
        components_per_dataset[dataset_name + "_training_times"] = model_training_times
    ...
    # Save model training times to CSV file
    training_times_name = "training_times_" + chemical_representation + ".csv"
    pd.DataFrame(components_per_dataset.items(), columns=['Dataset', 'Optimal Components']).to_csv(training_times_name, index=False)
